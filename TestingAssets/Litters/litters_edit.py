...
try:
	...
	lit_name_elem.clear()
	lit_name_elem.send_keys(new_rand_name);

	driver.find_element_by_id("LitterAdd").click()

	db.query("SELECT litter_name FROM litters where id="+litter_id)

	r=db.store_result()
	k=r.fetch_row(1,1)
	new_litter_name = k[0].get('litter_name')

	if new_rand_name == str(new_litter_name,"utf-8"):
		print("pass")
	else:
		print("fail")
